...
| # | Task | Status | Notes |
|---|------|--------|-------|
| 74 | Service account flag + create endpoint | Not started | Migration: `users.isService boolean default false`. New `POST /api/admin/service-accounts` body `{ name, role, color? }` — generates random password via crypto.randomBytes, calls `auth.api.createUser`, sets `isService: true`, returns user (no password in response). Filter `GET /api/users` to `WHERE isService=false` so the calendar member filter never shows bots. Tests: 401, 403, 400 invalid name, 200 create + flag set, /api/users excludes them, role honored |
| 75 | API key list metadata + rotation | Not started | Update `GET /api/admin/api-keys` to surface `lastRequest`, `requestCount`, `enabled`, `name`. New `POST /api/admin/api-keys/:id/rotate` — mints a new key with the same `name` + `userId`, returns plaintext, **does NOT auto-delete the old** (admin manually revokes after migrating the calling app — keeps a grace period without scheduled-deletion complexity). Tests: rotate creates new key, response has plaintext, old key still works until manually revoked, 401/403/404 |
| 76 | Service accounts list endpoint | Not started | New `GET /api/admin/service-accounts` returns shaped `[{ user, keys: [...] }]` for everyone with `isService=true` — single roundtrip avoids N+1 fetches in the UI. Tests: 401, 403, empty, populated with multiple keys per service |
| 77 | Tab switcher + Services list page | Not started | Refactor `app/admin/page.tsx` to render a tab switcher under the hero (`[Family][Services]`, `?tab=services` URL persistence). Family tab keeps existing portrait grid filtered to `!isService`. Services tab → new `services-grid.tsx` with **square cards** (1:1 aspect — visual rhythm break from family's portrait), inline SVG line-art icons (key/terminal/bot — signals "machine, not person"), Fraunces name, IBM Plex Mono masked prefix + activity row. `AddServiceCard` ghost dashed at the end. New `useServiceAccounts()` hook |
| 78 | Service Account drawer | Not started | New `service-account-drawer.tsx` reusing the `<Sheet>` primitive. Sections: header (large square line-art icon, inline name), Identity (email + role toggle + Active/Paused), **API keys** (list with monospace masked prefix + Fraunces italic last-request + tabular requestCount, per-row Rotate/Revoke, Mint button → inline form → reveal-once card with Copy + Check using the temp-password pattern), Danger zone (two-step name-confirm). New-mode creates the service then transitions into edit mode for immediate first-key minting |
| 79 | Polish + verification | Not started | Empty state for Services tab (Fraunces italic copy + inline SVG illustration + AddServiceCard), shimmer skeletons (4 square cards + monospace key rows), inline copy/check confirmation toasts, browser verify at 1440/1024/768/390px for both tabs and the drawer in both modes. Update `lessons.md` with the GitHub PAT mental model entry |
...
| # | Task | Status | Notes |
|---|------|--------|-------|
| 80 | Holidays service + endpoint | Not started | `apps/api/src/services/holidays.ts` with `getHolidays({ countries, from, to })` — iterates year ranges, filters `type === "public"`, dedupes by `date\|title`. `GET /api/holidays?countries=US,TW&from=&to=` with shared Zod schema. Per-request LRU cache for the `(countries, from, to)` tuple. Unit tests: year-boundary span, multi-country dedup, public-only filter, unknown country → 400 |
| 81 | User country preference | Not started | Migration: `users.holidayCountries text[]` nullable. `GET/PATCH /api/users/me/preferences` (hot path, not through Better Auth). Default on first visit from `Intl.DateTimeFormat().resolvedOptions().locale`. Shared `userPreferencesSchema` in `packages/shared`. Unit + integration tests |
| 82 | Kicker rendering in calendar views | Not started | New `useHolidays(countries, from, to)` hook. New `HolidayKicker` component: Fraunces italic lowercase, terracotta `·` prefix, absolute-positioned above the date numeral so it doesn't eat the 3-pill budget. Multi-country dedupes to one line with `·` separators. `day-cell.tsx` gets a `holidays` prop; `week-grid.tsx` + `day-grid.tsx` headers get the same kicker. Click → `EventDetailPopover` in new `readOnly` mode (title, country, type, no Edit) |
| 83 | Today view holiday line + settings UI | Not started | `today-view.tsx`: italic kicker between the weekday label and the massive date hero, only when today is a holiday. Event counts unaffected. Settings modal in `calendar-header.tsx`: new **Holidays** section with a multi-select country picker sourced from `date-holidays`' `getCountries()`, default from locale, saves via task-81 PATCH. "Show observances" checkbox wired to a no-op for v1. Browser verification at 1440/1024/768/390px on a known US holiday |
...
**Phase 16 complete + e2e shipped.** HomeCal's machine-callable surface is live and tested end-to-end. Next: **Phase 17 — Service Account & API Key Management UI** (tasks 74–79). Adds a "Services" tab to the existing /admin Family page so admins can create/monitor/rotate/revoke without curl. After that, Phase 18 is national holidays (tasks 80–83), Phase 19 is the deferred list.
