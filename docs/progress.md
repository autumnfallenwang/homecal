...
**Phase 14 complete** — all tasks 59–63 done. Today view is the default landing surface, complete with DST-correct backend endpoint, Morning Paper layout, polished state-aware timeline, relative-time glance rail, empty state with sun illustration, sticky compressed hero on scroll, and a11y live regions. Next: Phase 15 (Family page refresh with portrait grid + drawer + sessions API + temp password).
