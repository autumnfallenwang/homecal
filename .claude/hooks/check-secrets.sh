...
#      and `.env`/scratch paths that should never be committed.
...
if echo "$CMD" | grep -qE '([>]{1,2}\s*|tee\s+|sed\s+-i\s+.*\s|cp\s+\S+\s+|mv\s+\S+\s+)\.env([^.]|$)'; then
  echo 'BLOCKED: Bash command appears to write to a .env file. Secrets belong in deploy/.env.production (not committed) or your shell — not in committed .env files. Use .env.example for templates.' >&2
  exit 2
fi
...
SECRETS=$(git diff --cached --diff-filter=ACM -S 'password' -S 'secret' -S 'api_key' -S 'apikey' -S 'token' -S 'private_key' --name-only -- ':(exclude).claude/**' ':(exclude)*.md' ':(exclude)*.sh' 2>/dev/null)

# Known secret patterns (API keys, private keys). Exclude .md so doc examples
# don't trip. Anything actually encoding a key in source/config still matches.
PATTERNS=$(git diff --cached --diff-filter=ACM -G '(sk-[a-zA-Z0-9]{20,}|ghp_[a-zA-Z0-9]{36}|-----BEGIN (RSA |EC |DSA )?PRIVATE KEY-----|AKIA[0-9A-Z]{16})' --name-only -- ':(exclude)*.md' 2>/dev/null)

# .env files being committed (allow .env.example).
ENVFILES=$(git diff --cached --name-only 2>/dev/null | grep -E '\.env$|\.env\.' | grep -v '\.example$')

# Gitignored scratch paths staged (should never be committed).
SCRATCH=$(git diff --cached --name-only 2>/dev/null | grep -E '^(poc|ref|sandbox|scratch)/')

FOUND="${SECRETS}${PATTERNS}${ENVFILES}${SCRATCH}"

if [ -n "$FOUND" ]; then
  echo "BLOCKED: Potential secrets, .env files, or gitignored-folder files detected in staged files:" >&2
  echo "$FOUND" | sort -u >&2
